...
const PAGE_SIZE = 20;
const ESTIMATED_ROW_H = 120; // px – row height (112) + gap (8)
...
export default function DashboardContent({
  ...
  const {
    ...
    queryFn: async ({ pageParam = 0 }) => {
      ...
      if (statusFilter !== "ALL" && source === "user")
      ...
      return { items: json.items, hasMore: json.hasMore, offset: pageParam };
    },
    initialPageParam: 0,
      ...
      <main className="mx-auto max-w-5xl px-4 py-6">
        ...
        {mode === "library" && (
          <>
            ...
            {/* ---- error state ---- */}
            {status === "error" && (
              <p className="py-16 text-center text-[#E06B7A]">
              ...
              <div ref={listRef}>
                ...
                >
                  {virtualItems.map((virtualRow) => {
                    const isLoaderRow =
                      virtualRow.index > allItems.length - 1;
                    const anime = allItems[virtualRow.index];
                        ...
                        {isLoaderRow ? (
                          <div className="flex justify-center py-8">
                            {hasNextPage ? (
                              ...
                              <p className="text-sm text-[#6B6080]">
                          ...
                          <div
                            ...
                            className="group flex h-full cursor-pointer items-center gap-3 sm:gap-4 rounded-xl border border-[#2A2440]/60 bg-[#1A1625] p-3 transition-all duration-200 hover:scale-[1.015] hover:border-[#E064D6]/40 hover:bg-[#241E3A] hover:shadow-[0_4px_24px_rgba(224,100,214,0.12)]"
                          >
                            {/* cover image */}
                ...
                {/* end of list (when all loaded, no more pages) */}
